...
async def search_command(_, msg: Message):
    ...
    for item in results:
        if item['type'] == 'movie':
            ...
            label = 'FILE' if item.get('category') == 'direct_files' else 'MOVIE'
            stream_url = item['stream_url']
            download_url = item['download_url']
            api_url = item['api_url']

            link_line = f"📥 [Direct Download]({download_url})"
            if is_streamable:
                link_line = f"▶️ [Stream Online]({stream_url})  •  {link_line}"
            card = (
                f"🎬 **{title}**\n"
                f"📁 `{label}`  •  ⏱️ `{dur}`  •  💾 `{size}`\n"
                f"{link_line}\n"
...
async def set_title_command(_, msg: Message):
    ...
    if not new_title:
        return await msg.reply("❌ New title cannot be empty.", quote=True)
    ...
    for col, cat_name in ((db.anime, 'ANIME'), (db.webseries, 'WEB SERIES'), (db.movies, 'MOVIE')):
        if col is not None:
            batch_doc = await col.find_one({
                '$or': [
                    {'_id': target_key},
                    {'_id': {'$regex': f"^{escaped_key}", '$options': 'i'}},
                    {'title': {'$regex': escaped_key, '$options': 'i'}},
                    {'episodes.file_name': {'$regex': escaped_key, '$options': 'i'}},
                    {'file_name': {'$regex': escaped_key, '$options': 'i'}}
                ]
            })
