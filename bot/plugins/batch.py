...
def _flood_wait_seconds(exc: Exception) -> int | None:
    """Hydrogram/Pyrogram FloodWait compatibility without coupling to one release."""
...
async def _fetch_message_with_retry(channel_id, message_id, attempts: int = 3):
    last_error = None
    for attempt in range(1, attempts + 1):
        try:
            ...
            if attempt >= attempts:
                break
            delay = (flood_wait + 1) if flood_wait is not None else min(2 ** (attempt - 1), 5)
            logger.warning(
                "Retrying batch message %s/%s after %ss due to %s",
                channel_id,
                message_id,
                delay,
                exc,
...
async def batch_command(_, msg: Message):
        ...
        if other_args:
            ...
            if first in ('anime', 'animes'):
                ...
                return await msg.reply(
                    "❌ Movie batches are not supported. Use `/link <message_link>` for each movie. "
                    "`/batch` supports only `anime` and `series`.",
                    quote=True,
                )
            else:
                custom_title = " ".join(other_args).strip() or None
    ...
    for idx, curr_id in enumerate(range(start_id, end_id + 1), start=1):
        try:
            target_msg = await _fetch_message_with_retry(channel_id, curr_id)
            ...
            failed_ids.append(curr_id)
            logger.error("Failed to scan batch message %s/%s: %s", channel_id, curr_id, exc)

    if failed_ids:
        preview = ", ".join(str(x) for x in failed_ids[:20])
        more = f" (+{len(failed_ids) - 20} more)" if len(failed_ids) > 20 else ""
        return await status_msg.edit_text(
            "❌ **Batch indexing aborted safely.**\n\n"
            "Telegram/network errors prevented a complete scan, so **no incomplete batch was saved**.\n"
            f"Failed message IDs: `{preview}`{more}\n\n"
            "Run the same `/batch` command again after the connection/FloodWait clears."
        )
